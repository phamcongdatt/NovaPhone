...
<main class="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-10 sm:px-6">
    ...
    <div class="w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-night-soft/60 p-8 shadow-2xl shadow-black/50 backdrop-blur-2xl">
        <div class="text-center">
            <h1 class="text-3xl font-black tracking-tight text-white">Chào mừng trở lại</h1>
            <p class="mt-2 text-sm text-gray-400">Đăng nhập để quản lý đơn hàng và nhận ưu đãi riêng</p>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" class="mt-8 space-y-5">
            ...
            <div class="space-y-1.5">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('password.request') }}" class="text-xs font-bold text-brand-400 transition hover:text-brand-300">Quên mật khẩu?</a>
                </div>
                <div class="relative">
                    <input
                        id="password"
            ...
            <label class="flex items-center gap-2 text-sm text-gray-400">
                <input type="checkbox" name="remember" value="1" class="size-4 rounded border-white/10 bg-white/5 text-brand-600 focus:ring-brand-500/30">
                Ghi nhớ đăng nhập
            </label>
        ...
        <div class="relative my-6 flex items-center justify-center">
            ...
            <span class="relative bg-night-soft px-3 text-xs uppercase tracking-wider text-gray-500">Hoặc tiếp tục với</span>
        </div>
        ...
        <p class="mt-7 text-center text-sm text-gray-500">
            Chưa có tài khoản?
            <a href="{{ route('register') }}" class="font-bold text-brand-400 transition hover:text-brand-300">Đăng ký ngay</a>
        </p>
    </div>
</div>
</main>
