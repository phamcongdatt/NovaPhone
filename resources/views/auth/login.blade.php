...
<head>
    ...
    <title>Đăng nhập — NovaPhone</title>
    ...
    <style>
        ...
        .glow-border { position: relative; border-radius: 0; }
        .glow-border::before {
            content: '';
...
<body class="page-bg min-h-screen flex items-center justify-center p-4 sm:p-6 font-sans text-slate-200 antialiased">
    ...
    <div class="w-full max-w-6xl relative z-10">
        ...
        <div class="glow-border">
            <div class="bg-[#0b0b12]/95 backdrop-blur-xl overflow-hidden grid grid-cols-1 lg:grid-cols-2">

                {{-- ============ CỘT TRÁI: FORM ============ --}}
                <div class="relative z-10 p-7 sm:p-10">
                    <h2 class="text-2xl font-bold text-white mb-6 text-center">Đăng nhập tài khoản</h2>
                    ...
                    <form method="POST" action="{{ route('login') }}" novalidate class="space-y-4">
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            @if (Route::has('password.request'))
                                <a href="{{ route('password.request') }}" class="text-xs text-indigo-400 hover:text-indigo-300">Quên mật khẩu?</a>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center gap-3 my-5">
                        ...
                        <span class="text-xs text-slate-500">hoặc đăng nhập với</span>
                        <span class="h-px flex-1 bg-white/10"></span>
                    </div>
                    ...
                    <p class="text-center text-sm text-slate-400 mt-6">
                        Chưa có tài khoản?
                        <a href="{{ route('register') }}" class="text-indigo-400 hover:text-indigo-300 font-medium">Đăng ký</a>
                    </p>
                </div>

                {{-- ============ CỘT PHẢI: GIỚI THIỆU ============ --}}
                <div class="relative hidden lg:flex flex-col p-10 bg-gradient-to-r from-transparent via-indigo-950/20 to-blue-950/25">
                    <h2 class="text-3xl font-extrabold text-white leading-tight">Chào mừng trở lại</h2>
                    <p class="text-sm text-slate-400 mt-3 max-w-sm">
                        Đăng nhập để tiếp tục mua sắm, theo dõi đơn hàng và nhận
                        những ưu đãi độc quyền dành riêng cho thành viên NovaPhone.
                    </p>
        ...
        <div class="relative w-full max-w-sm overflow-hidden rounded-3xl border border-white/10 bg-[#0c0c14]/95 p-6 shadow-2xl shadow-indigo-500/10">
            ...
            <div class="space-y-3 pt-3 border-t border-white/10">
                ...
                <div class="flex gap-2">
                    ...
                    <button onclick="submitCustomMock()" class="rounded-xl bg-gradient-to-r from-indigo-600 to-blue-600 px-4 text-xs font-bold text-white hover:brightness-110 transition cursor-pointer">Đăng nhập</button>
                </div>
            </div>
        </div>
    </div>
