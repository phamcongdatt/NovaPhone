...
<div class="flex items-center justify-between mb-6">
    <form method="GET" action="{{ route('admin.post-categories.index') }}" class="relative w-full max-w-sm">
        ...
        <input name="search" value="{{ request('search') }}" type="search" placeholder="Tìm kiếm danh mục..." class="w-full rounded-xl border border-white/10 bg-[#0b1523] py-2.5 pl-10 pr-4 text-sm text-white outline-none placeholder:text-slate-600 focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50">
    </form>
    
    <a href="{{ route('admin.post-categories.create') }}" class="flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-blue-500">
        ...
        <table class="w-full text-left text-sm text-slate-300">
            <thead class="border-b border-white/10 bg-white/[0.02] text-xs font-semibold uppercase tracking-wider text-slate-400">
                <tr>
                    ...
                    <th class="px-5 py-4 text-center">Số bài viết</th>
                ...
                @forelse ($categories as $category)
                    <tr class="transition-colors hover:bg-white/[0.02]">
                        <td class="px-5 py-4">
                            ...
                            <p class="text-[11px] text-slate-500">Slug: {{ $category->slug }}</p>
                        </td>
                        <td class="px-5 py-4 text-center">
                            <span class="inline-flex size-6 items-center justify-center rounded-full bg-white/10 text-xs font-semibold text-white">{{ $category->posts_count }}</span>
                        </td>
                        <td class="px-5 py-4 text-center">
                        ...
                        <td class="px-5 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.post-categories.edit', $category) }}" class="rounded-lg p-2 text-slate-400 transition-colors hover:bg-white/5 hover:text-blue-400" title="Sửa">
                                ...
                                <form action="{{ route('admin.post-categories.destroy', $category) }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xoá danh mục này không?');" class="inline-block">
                                    @csrf
                                    @method('DELETE')
