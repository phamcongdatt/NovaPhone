            ...
            <div class="space-y-6 lg:col-span-2">
                ...
                <div>
                    ...
                    <textarea id="summary" name="summary" rows="3"
                              class="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">{{ old('summary', $post->summary) }}</textarea>
                ...
                <div>
                    ...
                    <textarea id="content" name="content" rows="15"
                              class="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">{{ old('content', $post->content) }}</textarea>
            ...
            <div class="space-y-6">
                <div class="rounded-xl border border-white/10 bg-white/[0.02] p-5">
                    ...
                    <label class="relative inline-flex cursor-pointer items-center">
                        <input type="checkbox" name="is_published" value="1" class="peer sr-only" {{ old('is_published', $post->is_published) ? 'checked' : '' }}>
                        <div class="peer h-6 w-11 rounded-full bg-slate-700 after:absolute after:left-[2px] after:top-[2px] after:h-5 after:w-5 after:rounded-full after:border after:border-gray-300 after:bg-white after:transition-all after:content-[''] peer-checked:bg-blue-600 peer-checked:after:translate-x-full peer-checked:after:border-white peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-800"></div>
                        <span class="ml-3 text-sm font-medium text-slate-300">Đã xuất bản</span>
                    </label>
                    
                    @if($post->published_at)
                    <div class="mt-3 text-xs text-slate-500">
                        Ngày đăng: {{ $post->published_at->format('d/m/Y H:i') }}
                    </div>
                    @endif
                </div>
                ...
                <div class="rounded-xl border border-white/10 bg-white/[0.02] p-5">
                    ...
                    <div class="flex flex-col items-center justify-center">
                        <label for="thumbnail" class="group relative flex h-48 w-full cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-white/20 bg-black/20 hover:border-blue-500/50 hover:bg-black/40 transition-all">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6 {{ $post->thumbnail ? 'hidden' : '' }}">
...
<script>
    function previewImage(input) {
        ...
        if (input.files && input.files[0]) {
            ...
            // Revert if no new file selected but we have an old one?
            // Actually it just stays as is.
        }
    }
</script>

<!-- Thêm CKEditor cho phần Nội dung -->
