...
<div class="mb-6 flex items-center justify-between">
    <a href="{{ route('admin.brands.index') }}" class="flex items-center gap-2 text-sm font-medium text-slate-400 transition-colors hover:text-white">
...
<div class="mx-auto max-w-2xl rounded-2xl border border-white/10 bg-[#0b1523] p-6 shadow-xl">
    <form action="{{ route('admin.brands.update', $brand) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="space-y-6">
            <div>
                <label for="name" class="mb-2 block text-sm font-medium text-slate-300">Tên thương hiệu <span class="text-red-500">*</span></label>
            ...
            <div>
                ...
                <p class="mt-1 text-[11px] text-slate-500">Bỏ trống nếu không muốn thay đổi logo.</p>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3"
                          class="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-2.5 text-sm text-white outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500">{{ old('description', $brand->description) }}</textarea>
            ...
            <div class="rounded-xl border border-white/10 bg-white/[0.02] p-5">
                ...
                <label class="relative inline-flex cursor-pointer items-center">
                    <input type="checkbox" name="is_active" value="1" class="peer sr-only" {{ old('is_active', $brand->is_active) ? 'checked' : '' }}>
